# adk-2.0-tutorial

Sample projects for learning **Google Agent Development Kit (ADK) 2.0**, built around an Enterprise Procurement Pipeline case study.

| Resource | Description |
|----------|-------------|
| [adk-procurement-workshop/README.md](adk-procurement-workshop/README.md) | Setup, three agents, quick start |
| [adk-procurement-workshop/ADK_2.0.md](adk-procurement-workshop/ADK_2.0.md) | ADK 2.0 concepts mapped to this repo |
| [adk-procurement-workshop/WORKSHOP_GUIDE.md](adk-procurement-workshop/WORKSHOP_GUIDE.md) | Optional ~60-minute presentation outline |

**Per-agent documentation:**

- [graph_procurement_agent/README.md](adk-procurement-workshop/graph_procurement_agent/README.md) — Graph workflow
- [dynamic_procurement_agent/README.md](adk-procurement-workshop/dynamic_procurement_agent/README.md) — Dynamic orchestrator
- [collaborative_procurement_agent/README.md](adk-procurement-workshop/collaborative_procurement_agent/README.md) — Collaborative coordinator

## Author

**Rohan Mitra** — Machine Learning Engineer & Researcher. Google Developer Expert — Cloud AI.

- Website: [rohanmitra.dev](https://rohanmitra.dev)
- LinkedIn: [linkedin.com/in/rohan-mitra14](https://www.linkedin.com/in/rohan-mitra14/)
